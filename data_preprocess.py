...
def preprocess_json_to_get_data(filepath, mode):
    ...
    data_dicts = {}
    all_ids_in_data = []
    print("preprocess_json_to_get_data:")
...
def get_word_index_from_span(tokens_passage,tokens_answers):
    ...
    answer_str = detokenizer(tokens_answers)

    for i in range(len_passage- len_answer+1):
        passage_str = detokenizer(tokens_passage[i:i+len_answer])
...
def form_embedding_matrix(filename, words):
    ...
    embeddings.append(numpy.random.ranf(300).tolist())
    embeddings.append(numpy.zeros(300).tolist())
...
def add_padding_and_remove_big_passages(dictionary, padding_index, max_question_length):
    print("add_padding_and_remove_big_passages")
    new_dict = {}
    for i in tqdm(dictionary):
...
if __name__ == "__main__":
    ...
    json_dev_file = 'dev-v2.0.json'
    json_train_file = 'train-v2.0.json'
