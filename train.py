...
if __name__ == "__main__":
    ...
    for i in bar:
        ...
        q.write("Epoch->" + str(i + 1 + start_from) + '\n')
        ...
        for batch in bar1:
            ...
            if batch % print_to_file_after_every == 0:
                ...
                if start_outputs[random_eg].item() <= end_outputs[random_eg].item():
                    ...
                    text_para = detokenizer(list_para )
                    text_ques = detokenizer(batch_token_questions[random_eg])

                    actual_answer = detokenizer(
                        list_para[start_spans[random_eg].item(): end_spans[random_eg].item() + 1] )
                    answer_string = detokenizer(
                        list_para[start_outputs[random_eg].item(): end_outputs[random_eg].item() + 1] )
                    ...
                    actual_answer = detokenizer(
                        list_para[start_spans[random_eg].item(): end_spans[random_eg].item() + 1] )
                    answer_string = detokenizer(
                        list_para[end_outputs[random_eg].item(): start_outputs[random_eg].item() + 1] )
        ...
        for batch in bar2:
            ...
            for j in range(len(batch_ids)):
                ...
                if start_outputs[j].item() <= end_outputs[j].item():
                    answer_str = detokenizer(
                        batch_text_para[j][start_outputs[j].item():end_outputs[j].item() + 1] )
                    results[str(batch_ids[j])] = str(answer_str)
                else:
                    answer_str = detokenizer(
                        batch_text_para[j][end_outputs[j].item():start_outputs[j].item() + 1] )
                    results[str(batch_ids[j])] = str(answer_str)
